...
const filteredProducts = computed(() => {
  ...
  return products.value.filter(
    (product) =>
      product.name
        ?.toLowerCase()
        .includes(filtersActive.value.global.value.toLowerCase()) ||
      product.code
        ?.toLowerCase()
        .includes(filtersActive.value.global.value.toLowerCase()) ||
      product.category
...
const clearSearch = () => {
  ...
  filtersActive.value.global.value = ""; //можно убрать, если не хотим обновление списка сразу при нажатии на крестик
};
const multiSelect = ref<MultiSelectInstance | null>(null);
